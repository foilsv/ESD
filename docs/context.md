...
5. Label editing is unambiguous intent: expose text controls without another navigation click. By default, Grouped label editing uses Flat's explicit one-line text row to avoid the larger Text popover; a saved Lab modifier restores the original automatic popover for comparison. Manually opening Text from the Grouped object row always remains a popover.
