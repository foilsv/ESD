...
- Follow the user-requested three solutions in `docs/solutions.md`. Colors use stateful compact dropdowns. The Interaction Lab's **Merge stroke color and style** modifier is on by default: Flat and Inline place stroke/line color inside the Stroke control, while Grouped keeps color independent; switching the modifier off makes color independent in all three. Flat Stroke, Text, and Alignment each replace the main row with a dedicated sub-toolbar and Back; Grouped uses compound popovers; Inline expands within the row. Do not replace these with the earlier Replace/Stack/Expand experiments.
